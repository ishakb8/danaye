...
if(isset($_POST['countrybtn'])){
	 ...
	 $update_query= mysql_query("UPDATE `country` SET `country_name`='$country ' WHERE country_id = '$country_id'");
	 if($update_query){
		 header("Location: country.php?updated=true");
		 }
	}
?>
...
<div class="container">
  ...
  <div class="wrapperDiv">
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="10" class="table">
      ...
      <?php $edit_query = mysql_query("SELECT * FROM `country` WHERE `country_id`='$country_id'");
	  while($edit_res= mysql_fetch_assoc($edit_query)){?>
      
        <tr>
        <td width="19%" height="30" align="left" valign="top" bgcolor="#FFFFFF" style="color:#6d6d6d;"><input type="text" name="country" value="<?php echo $edit_res['country_name'];?>" class="textBox" />
