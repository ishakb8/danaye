...
if ($count > 0) {
  ...
  while($res = mysql_fetch_assoc($fetch)) {
  	...
  	$substringtitle = substr($res['posting_title'],0,25);
	 
	  $image_res = mysql_fetch_array(mysql_query("SELECT * FROM postings_images WHERE posting_id= '".$res['posting_id']."'"));
	$output .= '<li><div style="padding:5px;"><div class="galleryBoxs"><div class="galleryImgDiv">';
	
	$output .= '<a  href="category-description.php?city_id=' . $city_id . '&categoty_id=' . $categoty_id . '&sub_categories_id=' . $sub_categories_id . '&posting_id=' . $res['posting_id'] . '"><img src="posting_uploads/'.$image_res['posting_paths'].'" width="275" height="150"/> </a>';
